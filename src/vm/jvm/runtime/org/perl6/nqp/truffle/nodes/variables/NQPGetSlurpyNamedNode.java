...
public class NQPGetSlurpyNamedNode extends NQPObjNode {
    private final FrameSlot slot;

    @CompilationFinal(dimensions = 1)
    private final String[] knownKeys;

    public NQPGetSlurpyNamedNode(FrameSlot slot, String[] knownKeys) {
        this.slot = slot;
    ...
    public Object execute(VirtualFrame frame) {
        NQPHash slurpy = new NQPHash();
        Object[] args = frame.getArguments();

        HashMap<String, Object> named = NQPArguments.getNamedArguments(args);
        
        if (named == null) {
            slurpy = new NQPHash();
        } else {
            HashMap<String, Object> cloned = (HashMap<String, Object>) named.clone();
            for (String knownKey : knownKeys) {
                cloned.remove(knownKey);
            }
            slurpy = new NQPHash(cloned);
        }
