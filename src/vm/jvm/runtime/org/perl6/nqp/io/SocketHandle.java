    ...
    public void setOption(ThreadContext tc, int option, long value) throws IOException {
        switch (option) {
            case SocketOptions.SO_KEEPALIVE:
                ...
                break;
            case SocketOptions.SO_LINGER:
                ((SocketChannel) chan).setOption(StandardSocketOptions.SO_LINGER, (int) value);
                break;
            case SocketOptions.SO_RCVBUF:
                ((SocketChannel) chan).setOption(StandardSocketOptions.SO_RCVBUF, (int) value);
                break;
            case SocketOptions.SO_REUSEADDR:
                ((SocketChannel) chan).setOption(StandardSocketOptions.SO_REUSEADDR, (value == 0) ? false : true);
                break;
            case SocketOptions.SO_SNDBUF:
                ((SocketChannel) chan).setOption(StandardSocketOptions.SO_SNDBUF, (int) value);
                break;
            case SocketOptions.TCP_NODELAY:
