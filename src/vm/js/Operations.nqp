class QAST::OperationsJS {
    ...
    add_simple_op('chown', $T_VOID, [$T_STR, $T_UINT64, $T_UINT64], :side_effects);
...
# HACK we need to check $body.type if we handle something like return
    ...
    add_simple_op('hllize', $T_OBJ, [$T_OBJ], :ctx, :side_effects, sub ($ctx, $obj) {
        "/*await*/ nqp.op.hllizefor($ctx, $obj, {quote_string($*HLL)})"
    });
    ...
    add_op('xor', sub ($comp, $node, :$want) {
        ...
        for @childlist -> $chunk {
            ...
            @setup.push($chunk);
            @setup.push(
                "if (/*await*/ {$chunk.expr}.\$\$decont($*CTX).\$\$toBool($*CTX)) \{\n"
                ~ "$true_count++;\n"
                ~ "$ret = {$chunk.expr};\n"
                ~ "\}\n"
            );
    ...
    add_simple_op('isprime_I', $T_INT, [$T_OBJ]);
