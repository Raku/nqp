...
sub nqp($prefix, $file, $stage, :$source=$prefix ~ '/' ~ $file ~ '.nqp', :$deps=[]) {
    ...
    rule($mbc, nqp::join(' ', $deps),
        make_parents($mbc),
        "\$(JS_NQP) --module-path=\$(JS_STAGE1) --target=mbc --output={nfp($mbc)} " ~ nfp($source),
...
constant('JS_NQP', nfp('./$(M_RUNNER)$(BAT)'));
...
rule('$(JS_CROSS_RUNNER)', '@script(gen-js-cross-runner.pl)@','$(PERL) @script(gen-js-cross-runner.pl)@ @nfp(./$(M_RUNNER))@');
...
rule('js-clean', '', '$(RM_RF) ' ~ nfp('$(JS_BUILD_DIR)/stage1 $(JS_BUILD_DIR)/stage2 $(JS_BUILD_DIR)/qregex.t ./package-lock.json ./node_modules $(BASE_DIR)/src/vm/js/nqp-runtime/node_modules'));
