...
Building the JavaScript backend currently requires an installed nqp-p.

    $ npm install src/vm/js/nqp-runtime-core src/vm/js/nqp-runtime-node src/vm/js/nqp-runtime


Currently it needs to be run like:

    $ nqp-p --module-path gen/js/stage1/ src/vm/js/bin/nqp-js.nqp -e 'nqp::say("Hello World")' | node
