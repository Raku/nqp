...
ok(join(',',$r.arguments) eq 'a,b,--target,c', '... and the right arguments');
...
ok(join(',', $r.arguments) eq 'script.pl,--verbose',
...
ok(join(',', $r.arguments) eq 'arg1,arg2',
...
$r := $x.parse(['-w', 'A', '-w', 'B']);
ok(join(',', $r.options<w>) eq 'A,B', 'multiple options with the same name');
