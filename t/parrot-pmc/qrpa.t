...
# Copyright (C) 2012, The Perl Foundation.

=head1 NAME

t/pmc/qrpa.t - testing the QRPA PMC
...
Tests C<QRPA> PMC. Checks size, sets various elements, including
...
.sub 'bool_tests'
    ...
    nok(p, "boolean pmc test empty")
    $I0 = istrue p
    nok($I0, "boolean int test empty")

    $P0 = box 'one'
    push p, $P0
    ok(p, "boolean test one elem")
    $I0 = istrue p
    ok($I0, "boolean int test one elem")

    $P0 = box 'two'
    push p, $P0
    ok(p, "boolean test two elem")

    $P0 = pop p
    ok(p, "boolean test after one pop")

    $P0 = pop p
    nok(p, "boolean test after one pop")
.end
...
.sub interface_check
    ...
    .local int b
    does b, p, "scalar"
    is(b, 0 ,"QRPA doesn't do scalar")
    does b, p, "array"
    is(b, 1, "QRPA does array")
    does b, p, "no_interface"
    is(b, 0, "QRPA doesn't do no_interface")
.end
...
.sub shift_pmc
    ...
    elem = shift pmc_arr
    is(elem, 4, "correct element shifted")
    elements = pmc_arr
    is(elements, 4, "correct element count after shifting")

    elem = shift pmc_arr
    is(elem, 3, "correct element shifted")
    elements = pmc_arr
    is(elements, 3, "correct element count after shifting")

    elem = shift pmc_arr
    is(elem, 2, "correct element shifted")
    elements = pmc_arr
    is(elements, 2, "correct element count after shifting")

    elem = shift pmc_arr
    is(elem, 1, "correct element shifted")
    elements = pmc_arr
    is(elements, 1, "correct element count after shifting")

    elem = shift pmc_arr
    is(elem, 0, "correct element shifted")
    elements = pmc_arr
    is(elements, 0, "correct element count after shifting")
...
loop:
    ...
    is($S1, "QRPA,", "QRPAs have the right MRO")
.end
...
.sub push_and_pop
    ...
    elements = pmc_arr
    is(elements, 0, "element count of empty QRPA is 0")
    ...
    f_elem = pop pmc_arr
    is(f_elem, 123.123000, "popped float is correct")

    i_elem = pop pmc_arr
    is(i_elem, 123, "popped int is correct")

    p_elem = pop pmc_arr
    is(p_elem, 456.456, "popped PMC is correct")

    s_elem = pop pmc_arr
    is(s_elem, "abc", "popped string is correct")
    elements = pmc_arr
    is(elements, 0, "element count after several pop operations is correct")
...
.sub test_assign_self
    ...
    arr = new ['QRPA']
    assign arr, arr
    ok(1, 'Can assign QRPA to itself')
.end
...
.sub test_assign_from_another
    ...
    assign arr1, arr2
    n = arr1
    is(n,15,'assigning to QRPA from another QRPA')
.end
